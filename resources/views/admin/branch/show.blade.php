<x-layout.admin>
    <div class="text-center flex flex-wrap gap-1">
        ...
        @if(auth()->user()->id == 1)
        <a href="{{ route('admin.branch.permission.index',$branch->id) }}" class="btn btn-sm btn-outline-primary">
            <x-svgicon.key />
            {{ __('permission') }}
        </a>
        @endif
        ...
        <!-- only allow superadmin -->
        @if(auth()->user()->id == 1)
        <a href="{{ route('admin.branch.manage_permission.index',$branch->id) }}" class="btn btn-sm btn-outline-primary">
            <x-svgicon.chain />
            {{ __('Manage Branches') }}
        </a>
        @endif
        ...
        <a href="{{ route('admin.impersonate.enable',$branch->id) }}" class="btn btn-sm btn-outline-primary">
    ...
    <div class="p-1 flex gap-3 items-center">
        @if($branch->phone)
        <a target="_blank" href="https://wa.me/{{ $branch->phone }}">
            <x-svgicon.whatsapp />
        </a>
        @endif
        <div class="text-gray-400 text-[10px]">{{ __('Phone') }}:{{ $branch->phone }}</div>
        @if(auth()->user()->id == 1)
        <div class="text-gray-400 text-[10px]">{{ __('Username') }}:{{ $branch->email }}</div>
        <div class="text-gray-400 text-[10px]">{{ __('Password') }}:{{ $branch->plain_password }}</div>
        @endif
    </div>
    ...
    <div class="text-center text-3xl">
        ...
        <div class="text-red-400 text-[10px]">{{ __('Moderator') }}</div>
        @endif
    </div>
