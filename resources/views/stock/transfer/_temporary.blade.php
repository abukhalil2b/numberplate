<div class="text-center text-2xl">{{ __('temporary') }}</div>

<form action="{{ route('stock.transfer.temporary_store') }}" method="post">
    @csrf

    <div class="bg-temporary p-3">
        <div class="flex gap-1">
            <span class="bg-green-400 w-64">{{ __('From Branch') }}: {{ app()->getLocale() == 'ar' ? $fromBranch->ar_name : $fromBranch->en_name}} {{ __('To Branch') }}:</span>
